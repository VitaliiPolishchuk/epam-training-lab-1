...
public class UserInterfaceServiceImpl implements UserInterfaceService {
    ...
    public void run() {
        System.out.printf("Enter age group(1-3), capital(170-5000), maximum toys allowed(7-20):\nAge group:\n\t1. %s\n\t2. %s\n\t3. %s\n",
                RoomImpl.AgeGroup.BABY,
                RoomImpl.AgeGroup.TEENAGE,
                RoomImpl.AgeGroup.ADULT);
        ...
        switch (Integer.parseInt(roomArgs[0])) {
            case 1:
                ageGroup = RoomImpl.AgeGroup.TEENAGE;
                break;
            case 2:
                ageGroup = RoomImpl.AgeGroup.TEENAGE;
                break;
            case 3:
                ageGroup = RoomImpl.AgeGroup.ADULT;
                break;
        }

        room = new RoomImpl(ageGroup,
